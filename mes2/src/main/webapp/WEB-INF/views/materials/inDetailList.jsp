...
<body>
	...
	<form action="/materials/inDetailList" method="get">
	...
	<div class="box-footer clearfix">
		<ul class="pagination pagination-sm no-margin pull-right">
			...
			<c:if test="${pageVO.prev }">
				<li><a
					href="/materials/inDetailList?page=${pageVO.startPage - 1 }">«</a></li>
			</c:if>

			<c:forEach var="i" begin="${pageVO.startPage }"
				end="${pageVO.endPage }" step="1">
				<li ${pageVO.cri.page == i?  "class='active'":"" }><a
					href="/materials/inDetailList?page=${i }"> ${i } </a></li>
			</c:forEach>

			<c:if test="${pageVO.next }">
				<li><a
					href="/materials/inDetailList?page=${pageVO.endPage + 1 }">»</a></li>
			</c:if>
		</ul>
	</div>
